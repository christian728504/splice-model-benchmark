...
import zarr
import sys
splice_transformer_path = os.path.join(os.getcwd(), 'reference_files', 'SpliceTransformer')
...
from sptransformer import Annotator #type: ignore

from utils import one_hot_encode

class SpliceTransformerEvaluator:
    def __init__(self,
                 ...
                 predicitons_path: str = 'results/splicetransformer_predictions.zarr',
                 splice_sites_path: str = 'reference_files/splice_sites.parquet',
                 aurpc_plot_path: str = 'results/splicetransformer.svg',
                 stratified_auprc_plot_path: str = 'results/splicetransformer_stratified_{type}.svg'):
    ...
    def _process_batch(self, batch, annotator):
        ...
        batch_tensor = torch.tensor(encoded_seqs, dtype=torch.float32).to('cuda')
        batch_tensor = batch_tensor.transpose(1, 2)

        with torch.no_grad():
            output = annotator.model.step(batch_tensor)
            output = output.cpu().numpy()

        for i, (chrom, index, strand) in enumerate(batch.keys()):
            acceptor_window = output[i, 1, :]
            donor_window = output[i, 2, :]
    ...
    def generate_sptransformer_predictions(self):
        ...
        for site in tqdm(self._splice_sites_df.iter_rows(named=True), total=len(self._splice_sites_df), desc="Processing sites"):
            ...
            if len(batch) == self.batch_size:
                self._process_batch(batch, annotator)
                batch = {}
